...
#' @param require.intersect If TRUE, stop if cellIDs do not overlap perfectly between metadata and filtered data. If FALSE, warn if cells are missing from either, but proceed with the intersection of cells. 
#' @param trim.suffix Remove the "-1" suffix if present in cell barcodes? 
...
gcdSoupX.load10X <-  function(dataDir,cellIDs=NULL,channelName=NULL,readArgs=list(),includeFeatures=c('Gene Expression'),verbose=TRUE,
                              require.intersect = T...){
  ...
  # if(!is.null(mDat)) {
  #   if (require.intersect) {
  #     if (length(rownames(mDat)!=length(colnames(datCells)))) {
  #       
  #     }
  #   }
  # } && any(rownames(mDat)!=colnames(datCells))){
